...
本仓库当前分支包含一组面向 WY 账号推荐与桌面 UI 的本地定制改动，主要目标是让 LX Music 可以读取 WY 登录态下的推荐内容，同时继续优先使用 LX Music 已配置的音源播放。

### 已完成

- 集成 `NeteaseCloudMusicApi` 作为主进程内的非官方 WY 接口封装，新增 WY 账号状态、二维码登录、退出登录、每日推荐、推荐歌单、歌单详情、喜欢歌曲同步等 IPC 能力。
- 左上角 LX LOGO 支持显示 WY 头像；点击头像或 LOGO 会弹出登录/登出菜单。推荐页面右上角的登录/退出按钮已移除，登录入口统一收敛到头像弹层。
- 推荐页改为歌单卡片布局，展示歌单封面、播放量、名称和描述；支持“查看全部”进入更多推荐。推荐列表会过滤公开推荐中的未登录私人雷达，登录态下优先展示账号相关推荐。
- WY 推荐歌单详情支持使用登录态接口拉取完整歌曲列表；私人雷达歌单会通过 WY 登录态详情接口获取，而不是使用未登录公开歌单详情。
- WY 推荐歌曲播放时不再使用 `NeteaseCloudMusicApi` 返回的音频 URL 作为主播放链路。播放前会根据歌曲名、歌手、专辑和时长调用 LX Music 内部多源匹配，优先切换到项目内已配置的可用音源。
- 将 WY 歌曲添加到“我的收藏”时，会继续按原始 WY 歌曲 ID 同步到 WY“我喜欢的音乐”。
...
- 对 WY 二维码登录、推荐歌单、私人雷达、收藏同步等流程补充自动化测试或可复现的端到端检查。
- 为 WY 推荐歌曲的多源匹配增加更明确的失败提示，必要时提供手动选择匹配结果的入口，避免同名歌曲误匹配。
- 继续验证 WY“我喜欢的音乐”取消收藏、歌单添加、多设备同步等边界行为，目前重点覆盖的是添加收藏时同步喜欢。
- 检查推荐歌单、最近播放、播放队列在大列表场景下的滚动性能和空状态细节。
- 为 WY 接口增加更细的错误分类与重试策略，例如登录过期、接口限流、歌单无权限、网络超时等。
- 便携版启动无窗口、后台残留进程等 Electron 打包问题仍需在真实安装/便携环境中继续排查。
- 如计划长期维护这组定制，建议把 WY 相关能力抽成更清晰的 feature 模块，并补充 README 中的登录、推荐、播放源配置使用说明。
